...
def build_report(all_rows: dict[str, list[Row]], configs: list[Config],
    ...
    L.append("\n## 四、结论（按实测数字，不做粉饰）\n")
    L.append(
        f"**1）修复打分覆盖缺陷后，混合与纯向量的差距收窄到 1 题。**\n"
        ...
        f"口语化 Hit@5 混合 {hit_at(coll, c_mix, 5)}/{len(coll)} 反超纯向量的 {hit_at(coll, c_vec, 5)}/{len(coll)}。"
        "修复前混合是 8/10 与 2/5，本次改动换来了这 2 题。\n\n"
        "**2）已修复的根因：关键词分数此前只覆盖关键词通道自己召回的候选。**\n"
        "融合时其余候选按 0 分参与加权——向量分全场最高的片段，综合分也会被硬扣"
        " (1-α) 份额而垫底（修复前实测：向量 0.791 的片段综合 0.553，排第 4）。\n"
        "现改为对所有候选的并集统一计算 BM25（retriever.py 融合段），界面上关键词列"
        "不再出现硬编码的 0。归一化仍按本候选集最大值，相对含义的问题依然存在，"
        "换 RRF 是下一步选项。\n\n"
        f"**3）互补性两个方向都有证据。**\n"
        f"术语型题组 Hit@5：混合 {hit_at(term, c_mix, 5)}/{len(term)}、"
        f"纯向量 {hit_at(term, c_vec, 5)}/{len(term)}、纯关键词 {hit_at(term, c_lex, 5)}/{len(term)}"
        "—— 向量兜住了关键词的漏。\n"
        f"口语化题组 Hit@5：混合 {hit_at(coll, c_mix, 5)}/{len(coll)}、"
        f"纯向量 {hit_at(coll, c_vec, 5)}/{len(coll)}、纯关键词 {hit_at(coll, c_lex, 5)}/{len(coll)}"
        "—— 关键词补上了向量的漏。两路互补成立，前提是两路的分数都能被公平计算。\n\n"
        f"**4）降级路径实测有效，且行为与修复前一致。**\n"
        f"把嵌入通道换成必然抛异常的桩后，Hit@1 = {hit_at(both, c_deg, 1)}/{len(both)}、"
        f"Hit@5 = {hit_at(both, c_deg, 5)}/{len(both)}，平均耗时 "
        f"{sum(r.per_config[c_deg]['ms'] for r in both) / max(1, len(both)):.0f} ms"
        "（省掉嵌入请求）。向量服务挂掉时退化为纯关键词检索，链路不断。\n\n"
        "**5）下一步可做的改进（按性价比排序）。**\n"
